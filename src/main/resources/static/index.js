...
function updateUser() {
    let roles
    let adminRole = document.getElementById("roles_editModal").value
    const id = document.getElementById("id_editModal").value
    if (adminRole.includes("1")) {
        roles = [
            {
                id: 1,
                "name": "ROLE_ADMIN"
...
function fillUserInformationPage(admin) {
...
function hideNavLinks() {
    const createUserBtn = document.getElementById("newUserNavLink")
    const usersTableBtn = document.getElementById("usersTableNavLink")
    createUserBtn.style.display = "none"
    usersTableBtn.style.display = "none"
}
